...
## Система порталов (КРИТИЧНО - не менять логику!)
**Цель:** 4 портала растут по очереди, игрок проходит один, остальные растут в фоне.

### Логика работы:
1. **При старте игры:**
   - Все 4 портала имеют статус `locked` (скрыты)
   - Создаётся очередь: [portal_1, portal_2, portal_3, portal_4]
   - Первый портал в очереди получает статус `growing`

2. **Фоновый рост (каждый тик, независимо от действий игрока):**
   - Если портал `growing` → время вышло → статус `active` (виден и кликабелен)
   - ЕслиPortal стал `active` → автоматически запускается следующий из очереди (`growing`)

3. **Игрок заходит в портал:**
   - `active` → карта боя → бой
   - **Рост других порталов продолжается в фоне**

4. **После боя (любой исход: победа/поражение/покинуть):**
   - Портал становится `locked` (скрыт)
   - Портал добавляется в **конец** очереди
   - Если есть `locked` в очереди и никто не растёт → запускаем следующий `growing`

### Статусы порталов:
- `locked` — скрыт (alpha=0), ждёт роста или в очереди
- `growing` — растёт (виден с анимацией, alpha=1)
- `active` — вырос (виден и кликабелен, alpha=1)

### Где хранится состояние:
- `GameState` (game_state.js) — очередь порталов + ID текущего растущего
- `PortalManager` (portal_manager.js) — данные статусов из portals.json
- `PortalRenderer` (portal_renderer.js) — отображение (alpha, анимации)
- `BaseScreen` (base_screen.js) — синхронизация при загрузке экрана

### Ключевые методы:
- `GameState.initFromPortals()` — инициализация очереди при первом запуске
- `GameState.syncGameStateFromPortals()` — синхронизация при возврате на базу (НЕ перезаписывает очередь!)
- `PortalManager.startNextPortalGrowth()` — запускает рост следующего из очереди
- `PortalManager.markPortalCompleted()` — портал завершён → locked + в конец очереди
- `PortalRenderer.update()` — фоновая проверка роста + обновление alpha

### Важно:
- Рост порталов **не останавливается**, когда игрок в бою
- Очередь сохраняется между переходами Base → Map → Base
- При возврате на базу: `syncGameStateFromPortals` только читает статусы, не перестраивает очередь
